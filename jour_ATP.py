...
def app():
    ...
    df_merged = pd.read_csv("ATP/df_merged.csv")
    ...
    odds30 = odds30.reset_index()
    result = pd.concat([odds30, test20], axis=1,ignore_index=False)
    result=result.replace({"home_player": inv_map})
    ...
    st.download_button(label="Download data as CSV",data=result,file_name='result.csv',mime='text/csv')
    ###############
    # VISUEL PLAYER
    ###############
    ...
    list_1 = list(result['home_player'].unique())
    list_2 = list(result['away_player'].unique())
